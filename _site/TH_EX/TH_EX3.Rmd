...
```{r}
packages = c('tidyverse', 'knitr', 'ggdist', 'scales', 'grid', 'gridExtra','formattable', 'patchwork', 'shiny','ggridges','gganimate','gifski','RColorBrewer','zoo')
...
Food <- financial %>%
  ...
  select(Dates,amount) %>%
  group_by(Dates) %>%
  summarise(Daily_food = sum(amount))
summary(Food)
```
...
Education <- financial %>%
  ...
  select(Dates,amount) %>%
  group_by(Dates) %>%
  summarise(Daily_Education = sum(amount))
summary(Education)
```
